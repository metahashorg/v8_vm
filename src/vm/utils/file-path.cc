...
FilePath::FilePath(const StringPieceType& path) {
  path_ = path ;
...
int FilePath::CompareIgnoreCase(StringPieceType string1,
  ...
  static decltype(::CharUpperA)* const char_upper_api =
      reinterpret_cast<decltype(::CharUpperA)*>(
          ::GetProcAddress(::GetModuleHandle(L"user32.dll"), "CharUpperA"));
  ...
  for ( ; i1 != string1end && i2 != string2end; ++i1, ++i2) {
    wchar_t c1 =
        (char)LOWORD(char_upper_api((LPSTR)(DWORD_PTR)MAKELONG(*i1, 0)));
    wchar_t c2 =
        (char)LOWORD(char_upper_api((LPSTR)(DWORD_PTR)MAKELONG(*i2, 0)));
    if (c1 < c2)
      return -1;
    if (c1 > c2)
      return 1;
...
Error OSErrorToVMError(DWORD last_error) {
  switch (last_error) {
...
Error CreateDirectory(const FilePath& full_path) {
  // If the path exists, we've succeeded if it's a directory, failed otherwise.
  const char* full_path_str = full_path.value().c_str() ;
  DWORD fileattr = ::GetFileAttributesA(full_path_str) ;
  if (fileattr != INVALID_FILE_ATTRIBUTES) {
    if ((fileattr & FILE_ATTRIBUTE_DIRECTORY) != 0) {
      V8_LOG_VBS(
          "CreateDirectory(\'%s\') - directory already exists", full_path_str) ;
      return errOk ;
    }

    return V8_ERROR_CREATE_WITH_MSG_SP(
        errPathNotDirectory,
        "CreateDirectory(\'%s\') - conflicts with existing file",
        full_path_str) ;
  }
  ...
  if (parent_path.value() == full_path.value()) {
    return V8_ERROR_CREATE_WITH_MSG_SP(
        errPathNotFound, "\'%s\' is failed", V8_FUNCTION) ;
  }

  Error result = CreateDirectory(parent_path) ;
  if (V8_ERROR_FAILED(result)) {
    V8_ERROR_ADD_MSG_SP(
        result, "Failed to create the parent directory - \'%s\'",
        parent_path.value().c_str()) ;
    return result ;
  }

  if (!::CreateDirectoryA(full_path_str, NULL)) {
...
#else  // defined(V8_OS_BSD) || defined(V8_OS_MACOSX) || defined(V8_OS_NACL)
typedef struct stat64 stat_wrapper_t ;

static int CallStat(const char *path, stat_wrapper_t *sb) {
  return stat64(path, sb) ;
}
#endif  // defined(V8_OS_BSD) || defined(V8_OS_MACOSX) || defined(V8_OS_NACL)

namespace {

Error OSErrorToVMError(int saved_errno) {
  switch (saved_errno) {
    case EACCES:
    case EISDIR:
    case EROFS:
    case EPERM:
      return errAccessDenied ;
    case EBUSY:
#if !defined(V8_OS_NACL)  // ETXTBSY not defined by NaCl.
    case ETXTBSY:
#endif
      return errFileInUse ;
    case EEXIST:
