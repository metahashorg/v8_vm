...
#include "vm_apps/third_party/url/scheme_host_port.h"
...
// @metahash #include "base/strings/string_number_conversions.h"
...
bool IsCanonicalHost(const std::string& host) {
  ...
  const Component raw_host_component(
      0, chromium::checked_cast<int>(host.length()));
...
bool IsValidInput(const std::string& scheme,
  ...
  bool is_standard = GetStandardSchemeType(
      scheme.data(),
      Component(0, chromium::checked_cast<int>(scheme.length())),
      &scheme_type);
  if (!is_standard)
    return false;
...
SchemeHostPort::SchemeHostPort(const GURL& url) : port_(0) {
  ...
  std::string scheme = url.scheme_piece();
...
std::string SchemeHostPort::SerializeInternal(url::Parsed* parsed) const {
  ...
  if (port_ != default_port) {
    result.push_back(':');
    std::string port(Uint32ToString(port_));
