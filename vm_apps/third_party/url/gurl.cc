...
GURL::GURL(std::string url_string) {
  InitCanonical(url_string, true);
}

GURL::GURL(std::wstring url_string) {
  InitCanonical(url_string, true);
}

GURL::GURL(const std::string& url_string, RetainWhiteSpaceSelector) {
  InitCanonical(std::string(url_string), false);
}
...
void GURL::InitCanonical(STR input_spec,
                         bool trim_path_end) {
...
std::string GURL::HostNoBracketsPiece() const {
...
bool GURL::DomainIs(std::string canonical_domain) const {
  if (!is_valid_)
    return false;
...
bool GURL::EqualsIgnoringRef(const GURL& other) const {
  ...
  return std::string(spec_).substr(0, ref_position) ==
         std::string(other.spec_).substr(0, ref_position_other);
}
...
bool operator==(const std::string& spec, const GURL& x) {
  return x == spec;
}
