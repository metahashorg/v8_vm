...
#include "vm_apps/third_party/url/url_constants.h"
...
class URL_EXPORT GURL {
 public:
  ...
  typedef url::StringPieceReplacements<std::wstring> ReplacementsW;
  ...
  explicit GURL(std::string url_string);
  explicit GURL(std::wstring url_string);
  ...
  GURL ReplaceComponents(
      const url::Replacements<wchar_t>& replacements) const;
  ...
  // colon.
  bool SchemeIs(std::string lower_ascii_scheme) const;
  ...
  std::string username_piece() const {
  ...
  std::string password_piece() const {
  ...
  std::string HostNoBracketsPiece() const;
  ...
  bool DomainIs(std::string canonical_domain) const;
 ...
 private:
  ...
  void InitCanonical(STR input_spec,
                     bool trim_path_end);
  ...
  std::string ComponentStringPiece(const url::Component& comp) const {
    if (comp.len <= 0)
      return std::string();
    return std::string(&spec_[comp.begin], comp.len);
  }
...
URL_EXPORT bool operator==(const GURL& x, const std::string& spec);
URL_EXPORT bool operator==(const std::string& spec, const GURL& x);
URL_EXPORT bool operator!=(const GURL& x, const std::string& spec);
URL_EXPORT bool operator!=(const std::string& spec, const GURL& x);
