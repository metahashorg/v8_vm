...
// @metahash #include "base/debug/leak_annotations.h"
...
using v8::vm::internal::LowerCaseEqualsASCII ;
...
template<> struct CharToStringPiece<wchar_t> {
  typedef std::wstring Piece;
};
...
bool IsStandard(const wchar_t* spec, const Component& scheme) {
  SchemeType unused_scheme_type;
  return DoIsStandard(spec, scheme, &unused_scheme_type);
}
...
bool DomainIs(std::string canonical_host,
  ...
  if (std::string(host_first_pos, canonical_domain.length()) !=
      canonical_domain) {
    return false;
  }
...
bool HostIsIPAddress(std::string host) {
